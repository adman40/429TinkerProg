...
void collectLabels(const char *fileName, HashMap *labelMap) {
    ...
    int memLabelCounter = 4096;  // Shared counter
    ...
    while (fgets(line, sizeof(line), inputFile)) {
        ...
        if (trimmed[0] == ':') {
            char label[50];
            // Read the token including the colon, e.g. ":D0"
            sscanf(trimmed, "%49s", label);
            trimWhitespace(label);  // Remove extra whitespace

            if (hashMapSearch(labelMap, label) == NULL) {  // Avoid duplicates
                char memAddressString[20];
                snprintf(memAddressString, sizeof(memAddressString), "%d", memLabelCounter);
                // Increment based on section: data labels increment by 8; code labels by 4.
...
char *parseFile(const char *fileName) {
    ...
    while (fgets(line, 255, inputFile)) {
        ...
        if (trimmed[0] == ';' || trimmed[0] == '\0' || trimmed[0] == '\t' || trimmed[0] == '\n') {
            ...
            if (existing == NULL) {
                // Label not already present; insert it.
                ...
                // Label already exists; use its stored address.
                strncpy(memAddressString, existing, sizeof(memAddressString));
                memAddressString[sizeof(memAddressString)-1] = '\0';
            }
            
            size_t lineLength = strlen(label) + strlen(memAddressString) + 4;  // For ':', space, '\n', and null terminator
            if (outputLength + lineLength >= bufferSize) {
                bufferSize *= 2;
                char *temp = realloc(outputBuffer, bufferSize);
                if (!temp) {
            ...
            if (isData) {
                // Skip the leading whitespace (tabs and spaces)
                char *trimmedLine = line;
                while (*trimmedLine == ' ' || *trimmedLine == '\t') {
                    trimmedLine++;
                }

                // Convert the trimmed line to a number.
                char *endPtr;
                ...
                // If there are any non-whitespace characters left and this is not a label,
                // then it’s an error.
                ...
                if (strcmp(opcode, "mov") == 0) {
                    ...
                    strcat(outputBuffer, line);
                    outputLength += lineLength;
                    continue;  // Skip validation for mov.
                }
                ...
                // Debugging extracted opcode and operands
                printf("DEBUG: Extracted opcode: '%s', operands: '%s'\n", opcode, operands);
                ...
                // Process operands based on expected format
                int opIndex = 0;
                ...
                // Check if the number of operands matches expectations
                ...
                // Handle instruction-specific translations
                if (strcmp(opcode, "clr") == 0) {
                    ...
                    rd[rdLength] = '\0';  // Null-terminate

                    // Trim leading spaces from label part
                    char *labelStart = commaPos + 1;
                    while (*labelStart == ' ') labelStart++;

                    // Extract label or immediate value
                ...
                // Append instruction to output buffer
